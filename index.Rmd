...
data %>%
  ...
  ggplot(aes(group = Park, x = Month, y = Year)) +
  
  geom_linerange(aes(y = Inf, xmin = -Inf, xmax = 2.5), color = "cyan", linewidth = 2) +
  geom_linerange(aes(y = Inf, xmin = 2.5, xmax = 5.5), color = "gold", linewidth = 2) +
  geom_linerange(aes(y = Inf, xmin = 5.5, xmax = 8.5), color = "chartreuse3", linewidth = 2) +
  geom_linerange(aes(y = Inf, xmin = 8.5, xmax = 11.5), color = "darkorange3", linewidth = 2) +
  geom_linerange(aes(y = Inf, xmin = 11.5, xmax = Inf), color = "cyan", linewidth = 2) +
  ...
  # geom_point( = 3, position = ggstance::position_dodgev(height = 1)) +
  # geom_jitter(aes(size = Area, color = Area)) +

  ggimage::geom_image(aes(image = Logo_url), size = 0.12, position = position_dodge(width = 1)) +
  # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_x_continuous(breaks = c(1:12), expand = expansion(mult = c(.1, .1))) +
  scale_y_reverse(breaks = c(2010:2100)) +
  coord_cartesian(xlim = c(1, 12)) +
  # scale_colour_gradientn(name = "Pinta-ala", colors = rev(c("#67001f", "#b2182b", "#d6604d", "#f4a582"))) +
  xlab("Kuukausi") + ylab("Vuosi")
